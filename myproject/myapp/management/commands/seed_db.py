...
from myapp.signals import (
    ...
    configurar_usuario,
    manejar_estado_reclamacion,
    registrar_accion_auditoria,
    actualizar_contador_pagos_contrato,

    # Señales de notificación
    notificar_creacion_usuario,
    notificar_creacion_intermediario,
    notificar_creacion_tarifa,
    notificar_creacion_afiliado_individual,
    notificar_creacion_afiliado_colectivo,
    notificar_creacion_contrato_individual,
    notificar_creacion_contrato_colectivo,
    notificar_creacion_factura,
    notificar_creacion_reclamacion
)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--intermediarios', type=int, default=75)
        parser.add_argument('--afiliados_ind', type=int, default=150)
        parser.add_argument('--afiliados_col', type=int, default=75)
        parser.add_argument('--tarifas', type=int, default=150)
        parser.add_argument('--contratos_ind', type=int, default=120)
        parser.add_argument('--contratos_col', type=int, default=60)
        parser.add_argument('--facturas', type=int, default=300)
        parser.add_argument('--reclamaciones', type=int, default=150)
        parser.add_argument('--pagos', type=int, default=225)
        parser.add_argument('--notificaciones', type=int, default=300)
        parser.add_argument('--auditorias', type=int, default=300)
    ...
    def handle(self, *args, **options):
        ...
        # --- LISTA DE SEÑALES A DESCONECTAR ---
        signals_to_manage = [
            ...
            # (pre_save, validar_afiliado_individual, AfiliadoIndividual), # <-- ELIMINAR
            # (pre_save, validar_reclamacion_basica, Reclamacion), # <-- ELIMINAR
            (post_save, manejar_estado_reclamacion, Reclamacion),
            # (pre_save, validar_intermediario_basico, Intermediario), # <-- ELIMINAR
            (post_save, registrar_accion_auditoria, AuditoriaSistema),
            # (pre_save, validar_tarifa, Tarifa), # <-- ELIMINAR
            (post_save, notificar_creacion_usuario, User),
            (post_save, notificar_creacion_intermediario, Intermediario),
            (post_save, notificar_creacion_tarifa, Tarifa),
            (post_save, notificar_creacion_afiliado_individual, AfiliadoIndividual),
            (post_save, notificar_creacion_afiliado_colectivo, AfiliadoColectivo),
            (post_save, notificar_creacion_contrato_individual, ContratoIndividual),
            (post_save, notificar_creacion_contrato_colectivo, ContratoColectivo),
            (post_save, notificar_creacion_factura, Factura),
            (post_save, notificar_creacion_reclamacion, Reclamacion),
            (post_save, actualizar_contador_pagos_contrato, Factura),
        ]

        # --- DESCONECTAR SEÑALES ---
        self.stdout.write(self.style.NOTICE(
            "Disconnecting signals for seeding..."))
        for signal, receiver, sender in signals_to_manage:
            try:
                signal.disconnect(receiver=receiver, sender=sender)
            except Exception as e:
                self.stdout.write(self.style.ERROR(
                    f"Error disconnecting {receiver.__name__} from {sender.__name__}: {e}"))

        try:
            # --- INICIO DEL CÓDIGO DE SEEDING ORIGINAL ---
            faker_locale = 'es_ES'
            try:
            ...
            if should_clean:
                ...
                try:
                    with transaction.atomic():
                        ...
                        self.stdout.write(
                            "  Deleting Intermediario objects...")
                        ...
                        User.objects.filter(
                            is_superuser=True, email='admin@example.com').delete()
                    ...
                    if clean_failed:
                        ...
                        # Reconnect signals before exiting
                        for signal, receiver, sender in signals_to_manage:
                ...
                tarifa_ids_db = list(
                    Tarifa.objects.values_list('pk', flat=True))
                ...
                factura_ids_db = list(
                    Factura.objects.values_list('pk', flat=True))
            ...
            def generate_historial_contrato():
                historial = []
                ...
                for _ in range(num_entries):
                    ...
                    val_ant = str(fake.pydecimal(
                        left_digits=5, right_digits=2)) if campo == 'suma_asegurada' else fake.word()
                    val_nue = str(fake.pydecimal(
                        left_digits=5, right_digits=2)) if campo == 'suma_asegurada' else fake.word()
                    ...
                    historial.append({'timestamp': historial_timestamp.isoformat(), 'usuario_id': usuario_id_historial,
                                     'campo': campo, 'anterior': val_ant, 'nuevo': val_nue, 'motivo': fake.sentence(nb_words=4)})
                return historial
            ...
            try:
                with transaction.atomic():
                    ...
                    try:
                        license_obj, created = LicenseInfo.objects.get_or_create(pk=LicenseInfo.SINGLETON_ID, defaults={
                                                                                 'license_key': nueva_license_key, 'expiry_date': fecha_expiracion_7_dias})
                        if created:
                            self.stdout.write(self.style.SUCCESS(
                    ...
                    if stats_m['requested'] > 0:
                        ...
                        for _ in range(stats_m['requested']):
                            try:
                                ...
                                fecha_app = fake.past_date(start_date="-3y")
                                tipo_f = random.choice(
                                    possible_fraccionamientos)
                                defaults = {'monto_anual': fake.pydecimal(left_digits=6, right_digits=2, positive=True, min_value=Decimal(
                                    '500.00'), max_value=Decimal('15000.00')), 'activo': fake.boolean(chance_of_getting_true=95)}
                    ...
                    for i in range(max(0, num_intermediarios_total)):
                        ...
                        try:
                            ...
                            inter = Intermediario(nombre_completo=fake.company() + " " + fake.company_suffix(), rif=intermediario_rif if has_rif else None, direccion_fiscal=fake.address() if fake.boolean(chance_of_getting_true=85) else None, telefono_contacto=generate_phone_ve(
                            ) if fake.boolean(chance_of_getting_true=95) else None, email_contacto=inter_email, porcentaje_comision=comision_directa_propia, porcentaje_override=porc_override_inter, intermediario_relacionado_id=padre_id_inter, activo=fake.boolean(chance_of_getting_true=95))
                            inter.save()
                            stats_m['created'] += 1
                            ...
                            if porc_override_inter > Decimal('0.00') and padre_id_inter is None:
                                parent_intermediario_pks_this_run.append(
                                    inter.pk)
                    ...
                    for _ in range(max(0, stats_m['requested'])):
                        ...
                        try:
                            ...
                            nacimiento = fake.date_of_birth(
                                minimum_age=0, maximum_age=90)
                            ingreso = fake.date_between(
                                start_date=nacimiento, end_date='today')
                    ...
                    if not (afiliado_ind_ids_created + afiliado_ind_ids_db) and stats_m['requested'] > 0:
                        try:
                            ...
                            afiliado_ind_ids_created.append(
                                emergency_af_ind.pk)
                            stats_m['created'] += 1
                        except Exception as e:
                            self.stderr.write(self.style.ERROR(
                    ...
                    if not (afiliado_col_ids_created + afiliado_col_ids_db) and stats_m['requested'] > 0:
                        try:
                            ...
                            afiliado_col_ids_created.append(
                                emergency_af_col.pk)
                            stats_m['created'] += 1
                        except Exception as e:
                            self.stderr.write(self.style.ERROR(
                    ...
                    if not all([AfiliadoIndividual.objects.exists(), Intermediario.objects.exists(), Tarifa.objects.exists()]):
                        ...
                        for i in range(max(0, stats_m['requested'])):
                            try:
                                ...
                                contrato_ind = ContratoIndividual(ramo=tarifa.ramo, forma_pago=random.choice([c[0] for c in CommonChoices.FORMA_PAGO]), estatus=estatus, suma_asegurada=fake.pydecimal(left_digits=6, right_digits=2, positive=True, min_value=Decimal(
                                    '5000.00')), fecha_emision=fecha_emision_dt_aware, fecha_inicio_vigencia=fecha_inicio, fecha_fin_vigencia=fecha_fin, intermediario=intermediario, tarifa_aplicada=tarifa, afiliado=afiliado, tipo_identificacion_contratante=afiliado.tipo_identificacion, contratante_cedula=afiliado.cedula, contratante_nombre=afiliado.nombre_completo, activo=True)
                                contrato_ind.save()
                                stats_m['created'] += 1
                        ...
                        for _ in range(max(0, stats_m['requested'])):
                            try:
                                af_col_principal = AfiliadoColectivo.objects.order_by(
                                    '?').first()
                                tarifa_cc = Tarifa.objects.order_by(
                                    '?').first()
                                ...
                                fecha_emision_dt_aware_cc = timezone.make_aware(
                                    datetime.combine(fecha_inicio, datetime.min.time()))
                                contrato_col = ContratoColectivo(ramo=tarifa_cc.ramo, forma_pago=random.choice([c[0] for c in CommonChoices.FORMA_PAGO]), estatus=estatus, suma_asegurada=fake.pydecimal(left_digits=7, right_digits=2, positive=True, min_value=Decimal(
                                    '50000.00')), fecha_emision=fecha_emision_dt_aware_cc, fecha_inicio_vigencia=fecha_inicio, fecha_fin_vigencia=fecha_fin, intermediario=intermediario_cc, tarifa_aplicada=tarifa_cc, razon_social=af_col_principal.razon_social, rif=af_col_principal.rif, cantidad_empleados=random.randint(5, 200), activo=True)
                                contrato_col.save()
                    ...
                    model_name = 'Factura'
                    stats_m = stats[model_name]
                    contratos_facturables = list(ContratoIndividual.objects.filter(estatus__in=[
                                                 'VIGENTE', 'VENCIDO'])) + list(ContratoColectivo.objects.filter(estatus__in=['VIGENTE', 'VENCIDO']))
                    if not contratos_facturables:
                        ...
                        for _ in range(max(0, stats_m['requested'])):
                            try:
                                ...
                                factura_instance = Factura(contrato_individual=contrato_obj_fact if isinstance(contrato_obj_fact, ContratoIndividual) else None, contrato_colectivo=contrato_obj_fact if isinstance(contrato_obj_fact, ContratoColectivo) else None, vigencia_recibo_desde=fecha_inicio_vigencia,
                                                           vigencia_recibo_hasta=fecha_fin_vigencia, intermediario=contrato_obj_fact.intermediario, monto=contrato_obj_fact.monto_cuota_estimada or Decimal('100.00'), dias_periodo_cobro=30, estatus_factura=estatus_factura, activo=True)
                                factura_instance.save()
                                stats_m['created'] += 1
                                if factura_instance.pk not in factura_ids_created:
                                    factura_ids_created.append(
                                        factura_instance.pk)
                    ...
                    stats_m = stats[model_name]
                    contratos_reclamables = list(ContratoIndividual.objects.filter(estatus__in=[
                                                 'VIGENTE', 'VENCIDO'])) + list(ContratoColectivo.objects.filter(estatus__in=['VIGENTE', 'VENCIDO']))
                        ...
                        for i_rec in range(max(0, stats_m['requested'])):
                            try:
                                ...
                                fecha_reclamo_val = fake.date_between_dates(
                                    date_start=fecha_inicio_valida, date_end=fecha_fin_valida)
                                ...
                                if estado_actual_rec in estados_que_implican_cierre:
                                    fecha_cierre_rec = fake.date_between_dates(
                                        date_start=fecha_reclamo_val, date_end=date.today())
                                suma_asegurada_contrato = contrato_obj_rec.suma_asegurada or Decimal(
                                    '1000.00')
                                monto_reclamado_val = fake.pydecimal(left_digits=int(len(str(int(suma_asegurada_contrato)))) - 1 if len(
                                    str(int(suma_asegurada_contrato))) > 1 else 1, right_digits=2, positive=True, min_value=Decimal('50.00'))
                                if monto_reclamado_val >= suma_asegurada_contrato:
                                    monto_reclamado_val = suma_asegurada_contrato * \
                                        Decimal('0.5')
                                reclamacion_instance = Reclamacion(contrato_individual=contrato_obj_rec if isinstance(contrato_obj_rec, ContratoIndividual) else None, contrato_colectivo=contrato_obj_rec if isinstance(contrato_obj_rec, ContratoColectivo) else None, tipo_reclamacion=random.choice([c[0] for c in CommonChoices.TIPO_RECLAMACION]), estado=estado_actual_rec, descripcion_reclamo=fake.paragraph(
                                    nb_sentences=3), monto_reclamado=monto_reclamado_val.quantize(Decimal('0.01')), fecha_reclamo=fecha_reclamo_val, fecha_cierre_reclamo=fecha_cierre_rec, primer_nombre=f"Reclamo para Contrato", primer_apellido=f"{contrato_obj_rec.numero_contrato or contrato_obj_rec.pk}")
                    ...
                    if not facturas_pagables.exists():
                        ...
                        for _ in range(max(0, stats_m['requested'])):
                            try:
                                ...
                                monto_pago = (pendiente * Decimal(random.uniform(0.2, 0.8))).quantize(
                                    Decimal('0.01')) if es_parcial else pendiente
                                monto_pago = max(Decimal('0.01'), monto_pago)
                                aplica_igtf = random.random() < (igtf_chance / 100.0)
                                pago = Pago.objects.create(factura=factura_a_pagar, monto_pago=monto_pago, fecha_pago=fecha_pago, forma_pago=random.choice(
                                    [c[0] for c in CommonChoices.FORMA_PAGO_RECLAMACION]), referencia_pago=fake.bothify(text='REF-?#?#?#?#'), aplica_igtf_pago=aplica_igtf, activo=True)
                                stats_m['created'] += 1
                        ...
                        for pago in pagos_validos_para_comision.select_related('factura__intermediario__intermediario_relacionado'):
                            try:
                                ...
                                if intermediario_directo.porcentaje_comision > 0:
                                    ...
                                    RegistroComision.objects.create(intermediario=intermediario_directo, pago_cliente=pago, factura_origen=pago.factura, contrato_individual=pago.factura.contrato_individual, contrato_colectivo=pago.factura.contrato_colectivo,
                                                                    tipo_comision='DIRECTA', porcentaje_aplicado=intermediario_directo.porcentaje_comision, monto_base_calculo=pago.monto_pago, monto_comision=monto_comision.quantize(Decimal('0.01')))
                                    stats_m['created'] += 1
                                ...
                                if intermediario_padre and intermediario_padre.porcentaje_override > 0:
                                    ...
                                    RegistroComision.objects.create(intermediario=intermediario_padre, pago_cliente=pago, factura_origen=pago.factura, contrato_individual=pago.factura.contrato_individual, contrato_colectivo=pago.factura.contrato_colectivo,
                                                                    tipo_comision='OVERRIDE', porcentaje_aplicado=intermediario_padre.porcentaje_override, monto_base_calculo=pago.monto_pago, monto_comision=monto_override.quantize(Decimal('0.01')), intermediario_vendedor=intermediario_directo)
                                    stats_m['created'] += 1
                    ...
                    if not available_user_pks_audit:
                        ...
                        for _ in range(max(0, stats_m['requested'])):
                            try:
                                ...
                                audit = AuditoriaSistema(usuario_id=usuario_audit_id, tipo_accion=random.choice([a[0] for a in CommonChoices.TIPO_ACCION]), resultado_accion=random.choice([r[0] for r in CommonChoices.RESULTADO_ACCION]), tabla_afectada=random.choice(
                                    [None, "users_usuario", "myapp_factura"]), registro_id_afectado=random.randint(1, 1000) if random.choice([True, False]) else None, tiempo_final=audit_tiempo_final)
                                audit.save()
                                stats_m['created'] += 1
                            except Exception as e:
                                stats_m['failed'] += 1
            ...
            model_order_display = ['LicenseInfo', 'Tarifa', 'User', 'Intermediario', 'AfiliadoIndividual', 'AfiliadoColectivo',
                                   'ContratoIndividual', 'ContratoColectivo', 'Factura', 'Reclamacion', 'Pago', 'RegistroComision', 'Notificacion', 'AuditoriaSistema']
            ...
            for model_key in model_order_display:
                if model_key in stats:
                    ...
                    elif ((requested_val > 0 and actual_created >= requested_val) or (model_key == 'RegistroComision' and actual_created > 0) or (model_key == 'LicenseInfo' and actual_created == 1)) and s.get('failed', 0) == 0:
            ...
            # --- RECONECTAR SEÑALES (MUY IMPORTANTE) ---
            self.stdout.write(self.style.NOTICE("Reconnecting signals..."))
            for signal, receiver, sender in signals_to_manage:
                try:
                    signal.connect(receiver=receiver, sender=sender)
                except Exception as e:
                    self.stdout.write(self.style.ERROR(
                        f"Error reconnecting {receiver.__name__} to {sender.__name__}: {e}"))
